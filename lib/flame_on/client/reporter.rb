module FlameOn
  module Client
    class Reporter
      def initialize(adapter:, processor:, max_queue_size:, max_batch_size:, flush_interval_ms:, drop_policy:)
        @adapter = adapter
        @processor = processor
      ...
      def start
        @worker_mutex.synchronize do
          ...
          @worker.name = 'flame_on_reporter' if @worker.respond_to?(:name=)
        end
      end
      ...
      def ship(batch)
        ...
        @adapter.send_batch(processed_batch)

        @stats_mutex.synchronize do
          @stats[:batches_sent] += 1
          @stats[:traces_sent] += processed_batch.length
        end
